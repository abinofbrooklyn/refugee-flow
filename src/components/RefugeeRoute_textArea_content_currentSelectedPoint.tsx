...
interface SelectedDataPoint {
  ...
  dead_and_missing?: string | number;
  description?: string;
  source?: string;
  lng?: number | null;
  lat?: number | null;
...
`;
const Delimiter = styled.div<{ offset?: string }>`
  width: 100%;
  height: 1px;
  transition: all 200ms;
  background: #76819ea3;
...
const StatsBoardWrapper = styled.div`
  ...
  flex-shrink: 0;
`;
const StatsBoardItem = styled.div<{ index?: number; name?: string }>`
  height: 40px;
  background: #2f2f4ab3;
  border-radius: 4px;
  position: relative;
...
const RefugeeRoute_textArea_content_currentSelectedPoint: React.FC<Props> = ({ selected_dataPoint }) => {
  const mapContainerRef = useRef<HTMLDivElement>(null);
  const mapRef = useRef<maplibregl.Map | null>(null);

  const parsedPoint: SelectedDataPoint | null = selected_dataPoint ? JSON.parse(selected_dataPoint) : null;

  const mapFly = (map_lng: number, map_lat: number) => {
    if (!mapRef.current) return;
    const source = mapRef.current.getSource('point') as maplibregl.GeoJSONSource | undefined;
    if (source) {
  ...
  useEffect(() => {
    ...
    const map_lng = parsedPoint ? (parsedPoint.lng ?? 0) : 0;
    const map_lat = parsedPoint ? (parsedPoint.lat ?? 0) : 0;
    ...
    mapRef.current.on('load', () => {
      ...
      mapFly(map_lng as number, map_lat as number);
    });
  ...
  // Fly when selected_dataPoint changes
  useEffect(() => {
    if (!mapRef.current || !selected_dataPoint) return;
    const point: SelectedDataPoint = JSON.parse(selected_dataPoint);
    const map_lng = point ? (point.lng ?? 0) : 0;
    const map_lat = point ? (point.lat ?? 0) : 0;
    mapFly(map_lng as number, map_lat as number);
  }, [selected_dataPoint]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <Wrapper>
      <HeaderSection>
        <Cause_of_death><em>Cause of Death</em> {parsedPoint && (parsedPoint.cause_of_death_displayText + '/ ' + parsedPoint.cause_of_death)}</Cause_of_death>
      ...
      <LocationRow>
        ...
        <Route>Route: {parsedPoint && parsedPoint.route_displayText as string}</Route>
      </LocationRow>
      <LocationDesc>{parsedPoint && parsedPoint.location as string}</LocationDesc>
      <div style={mapStyle} ref={mapContainerRef} />
      <StatsBoardWrapper>
        <StatsBoardItem index={1} name='Dead: '><p>{parsedPoint && parsedPoint.dead as string}</p></StatsBoardItem>
        <StatsBoardItem index={2} name='Missing: '><p>{parsedPoint && parsedPoint.missing as string}</p></StatsBoardItem>
        <StatsBoardItem index={3} name='Dead and Missing: '><p>{parsedPoint && parsedPoint.dead_and_missing as string}</p></StatsBoardItem>
      </StatsBoardWrapper>

      {parsedPoint && parsedPoint.description && (
        <>
          <Delimiter offset={'15px'}/>
          <MediaSectionWrapper>
...
export default RefugeeRoute_textArea_content_currentSelectedPoint;
