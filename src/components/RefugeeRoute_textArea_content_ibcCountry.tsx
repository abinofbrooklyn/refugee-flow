...
`;
const IBCTitle = styled.div<{ currentRouteName?: string }>`
  top: -35px;
  position: relative;
  &>p{
    position: relative;
...
`;
const SearchBar = styled.div<{ inputOn?: boolean }>`
  position: absolute;
  top: 0px;
  width: 20px;
  height: 20px;
...
`;
const SearchBarInput = styled.input<{ inputOn?: boolean }>`
  background: none;
  opacity: 0;
  border: none;
...
`;
const SearchBarInputTips = styled.p<{ inputOn?: boolean }>`
  opacity: ${props => props.inputOn ? 1 : 0};
  position: absolute;
  right: 30px;
  top: 20px;
  ...
  font-weight: 300;
`;
const SearchDropDown = styled.ul<{ paddingToggle?: HTMLInputElement | null; inputOn?: boolean }>`
  color: white;
  font-size: 10px;
  font-family: 'Roboto';
  font-weight: 300;
...
`;
const Tags = styled.div<{ inputOn?: boolean }>`
...
const RefugeeRoute_textArea_content_ibcCountry: React.FC<Props> = ({ IBC_data, currentRouteName }) => {
  ...
  const fuseRef = useRef<Fuse<{ key: string }> | null>(null);
  const cardItemAllRef = useRef<React.ReactElement[]>([]);

  const cardGenerator = (): Map<number, React.ReactElement[]> => {
    const ibcKey = currentRouteName === 'Others' ? 'Other' : currentRouteName;
    const ibcRouteData = ibcKey ? IBC_data[ibcKey] : undefined;
    if (!ibcRouteData) {
      cardItemAllRef.current = [];
      return new Map();
    }

    const cardItemAll = (ibcRouteData as Record<string, unknown>[]).map(data => {
      const d = { ...data } as Record<string, unknown> & {
        chartData: Array<{ value: number; key: string }>;
        totalCross: number;
        NationalityLong: string;
      };
      d.chartData = [];
      ...
      const dataYears = Object.keys(d).filter(k => /^\d{4}$/.test(k)).sort();
      dataYears.forEach(key => {
        const yearlyTotal = d[key];
        if (yearlyTotal && typeof yearlyTotal === 'object') {
          const res = Object.values(yearlyTotal as Record<string, number | null>).reduce(
            (a: number, c) => a + (c || 0), 0
          );
          d.chartData.push({ value: res, key });
        }
      });
      d.totalCross = d.chartData.reduce((a, c) => ({ value: a.value + c.value }), { value: 0 })['value'];

      return (
        <RefugeeRoute_textArea_content_ibcCountryItem
          data={d as Parameters<typeof RefugeeRoute_textArea_content_ibcCountryItem>[0]['data']}
          key={d['NationalityLong'] as string}
        />
      );
    });

    cardItemAllRef.current = _.sortBy(cardItemAll, d => d.props.data['totalCross']).reverse();

    const cardItemSub = (() => {
      if (cardItemAllRef.current.length > 10) {
        const temp = new Map<number, React.ReactElement[]>();
        let pageCounter = 1;
        for (let i = 0; i < cardItemAllRef.current.length; i += 10) {
          const page = cardItemAllRef.current.slice(i, i + 10);
          if (page.length > 0) {
        ...
        const temp = new Map<number, React.ReactElement[]>();
        temp.set(1, cardItemAllRef.current);
        return temp;
      }
    })();

    if (inputOn && searchedItem.length > 0) {
      const filtered = searchedItem
        .map(i => _.find(cardItemAllRef.current, d => d.key === i))
        .filter((d): d is React.ReactElement => d != null);
      return new Map<number, React.ReactElement[]>().set(1, filtered);
    } else {
      return cardItemSub;
    }
  };
  ...
  const handlePageClick = (param: 'up' | 'down', pageMax: number) => {
  ...
  return (
    <Wrapper className='route-map-titleGroup__IBC'>
      <IBCTitle currentRouteName={currentRouteName}><p>Illegal Border Crossing Involved Country</p></IBCTitle>

      <DataSource onClick={() => window.open(
      ...
      />
      <SearchBarInputTips inputOn={inputOn}>*Multi Selection</SearchBarInputTips>

      <SearchDropDown
        paddingToggle={searchBarRef.current}
        inputOn={inputOn}>
        {dropDownItem.map(item => (
          <li key={item} onClick={() => {
            if (!searchedItem.includes(item)) {
      ...
      <Tags inputOn={inputOn}>
        {searchedItem.map(item => (
          <div key={item}
            onClick={() => setSearchedItem(prev => prev.filter(d => d !== item))}
          >
            <span>x</span>
            <p>{item}</p>
          </div>
      ...
      <CardContainer id="CardContainer">{cards.get(currentPage)}</CardContainer>

      <PageUp onClick={() => handlePageClick('up', pageMax)}>{"<"}</PageUp>
      <Page>{currentPage + '/' + pageMax}</Page>
      <PageDown onClick={() => handlePageClick('down', pageMax)}>{">"}</PageDown>

    </Wrapper>
  );
};

export default RefugeeRoute_textArea_content_ibcCountry;
