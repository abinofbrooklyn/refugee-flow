...
**Goal:** Delete exposed credential backup, patch npm vulnerabilities, upgrade Helmet to v8 with Content-Security-Policy, sanitize error responses, restrict CORS to GET/HEAD, tighten rate limiting on infrastructure endpoints
**Requirements**: Security maintenance (no formal REQ-IDs)
**Depends on:** None (security work is independent)
**Plans:** 2/2 plans complete

Plans:
- [ ] 12-01-PLAN.md — Upgrade Helmet v8, remove unused deps (multer, @types/helmet), npm audit fix, delete credential backup, update .env.example
- [ ] 12-02-PLAN.md — Configure Helmet v8 CSP, CORS GET-only, health rate limiter, error sanitization, security tests
### Phase 13: Route Map Rendering Performance

**Goal:** Reduce CPU/GPU usage during map interaction — throttle canvas redraws with requestAnimationFrame, optimize sizeScaler and quadtree rebuilds to prevent battery drain on laptops and old machines
**Requirements**: TBD
**Depends on:** Phase 10
**Plans:** 0 plans
...
- [ ] 14-02-PLAN.md — Create CloudFormation template (VPC, ECS, ECR, S3, CloudFront, IAM, monitoring) and task definition JSON
- [ ] 14-03-PLAN.md — Create GitHub Actions CI/CD workflow with OIDC authentication
- [ ] 14-04-PLAN.md — Deploy CloudFormation stack, populate secrets, verify end-to-end operation (checkpoint)

### Phase 15: Responsive Display Audit
